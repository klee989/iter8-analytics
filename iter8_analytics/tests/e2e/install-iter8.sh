...
if [ "$MIXER_DISABLED" = "false" ]; then
  ...
  kubectl apply -f https://raw.githubusercontent.com/iter8-tools/iter8-controller/v1.0.0-rc3/install/iter8-controller.yaml
else
  echo "Using Istio telemetry v2"
  if (( -1 == "$(semver_compare ${ISTIO_VERSION} 1.7.0)" )); then
    kubectl apply -f https://raw.githubusercontent.com/iter8-tools/iter8-controller/v1.0.0-rc3/install/iter8-controller-telemetry-v2.yaml
  else
    kubectl apply -f https://raw.githubusercontent.com/iter8-tools/iter8-controller/v1.0.0-rc3/install/iter8-controller-telemetry-v2-17.yaml
  fi
fi
